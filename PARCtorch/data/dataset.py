...
def validate_data_format(
    ...
    for data_dir in data_dirs:
        ...
        # Use glob and filter out JSON files explicitly
        dir_files = sorted(glob.glob(os.path.join(data_dir, "*.npy")))
        
        # Double-check to filter out any JSON files
        dir_files = [f for f in dir_files if not f.endswith('.json') and f.endswith('.npy')]
        
        if not dir_files:
            logging.warning(f"No .npy files found in directory '{data_dir}'.")
        else:
            logging.info(f"Found {len(dir_files)} .npy files in '{data_dir}'.")
    ...
    logging.info(f"Total .npy files to validate: {len(all_files)}")
    ...
    if required_channels is not None:
        ...
        logging.info(f"Number of channels detected: {num_channels}")
    ...
    logging.info(f"✓ Data validation completed successfully. All {len(all_files)} files are valid.")
...
class GenericPhysicsDataset(Dataset):
    ...
    def normalize_channel(self, tensor, channel_idx):
        ...
        if max_val - min_val == 0:
            # Channel is constant (all same value), no normalization needed
            # Subtract min to center at 0 if the constant value is not already 0
